...
bool is_valid_time(const std::string& str) {
	unsigned int strSize = str.size();
	bool decimalPlace = false; //Makes sure there is only one '.'
	bool containsDigit = false; //Makes sure the input isn't only a '.'
	if(str[0]=='-' || strSize==0){
		return false;
	}
	for(unsigned int i=0; i<strSize; i++){
		if(isdigit(str[i]) && !containsDigit){
			containsDigit = true;
		}
		if(str[i]=='.' && !decimalPlace){
			decimalPlace = true;
		}else if(str[i]=='.' && decimalPlace){
			return false;
		}
...
{
	float compare; //time to compare to
	int greaterThan; //number of times the compare time is greater than
	for(unsigned int i=0; i<SIZE; i++){
		compare = timeArray[i];
		greaterThan = 1;
		for(unsigned int j=0; j<SIZE; j++){
			if(compare>timeArray[j]){
				greaterThan++;
			}
		}
		rankArray[i]=greaterThan;
	}
}
